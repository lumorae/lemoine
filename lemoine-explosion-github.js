...
function initLemoineExplosion() {
  ...
  const lemonPlaneMat = new THREE.MeshBasicMaterial({
    map: lemonTex,
    transparent: true,
  });
  ...
  const lemonIconMesh = new THREE.InstancedMesh(lemonIconGeom, lemonIconMat, LEMON_ICON_COUNT);
  ...
  function createMetatronGeometry() {
    ...
    for (let i = 0; i < nodes.length; i++) {
      addCircle(points, nodes[i].x, nodes[i].y, 0.16, 42);
    }
  ...
  function spawnFlat(idx, ox, oy) {
    ...
    if (sr < 0.5) {
      flat.sizes[idx] = 0.3 + Math.random() * 0.5;
    } else if (sr < 0.85) {
      flat.sizes[idx] = 1.0 + Math.random() * 1.0;
    } else {
  ...
  function spawnCube(idx, ox, oy) {
    ...
    if (isStriped) {
      cube.sizes[idx] = 0.14 + Math.random() * 0.16;
    } else {
      const sr = Math.random();
    ...
    if (isStriped) {
      const sr = Math.random();
      if (sr < 0.35) c = [0.8, 0.21, 0.4];
      else if (sr < 0.58) c = [0.9, 0.68, 0.22];
  ...
  function spawnLemonIcon(idx) {
    ...
    const c = Math.random() < 0.55 ? [0.8, 0.21, 0.4] : pickDigitalColor();
  ...
  let autoTimer = 0;
  let nextAuto = 26 + Math.random() * 18;
  ...
  function ambientBurst() {
    ...
    const x =
      side < 0.34
        ? -halfWidthWorld * (0.45 + Math.random() * 0.45)
        : side < 0.72
          ? halfWidthWorld * (0.25 + Math.random() * 0.65)
          : (Math.random() - 0.5) * halfWidthWorld * 0.55;

    const y = (Math.random() - 0.5) * 1.65;
    const organic = computeProbOrganic(x);

    emit(x, y, 110 + Math.floor(Math.random() * 120), organic);

    if (1 - organic > 0.35 && Math.random() < 0.65) {
      emitSacred(x, y, 1 + Math.floor(Math.random() * 2), 1 - organic);
    }
  }
  ...
  function updateSacred(dt, t) {
    for (let i = 0; i < SACRED_COUNT; i++) {
      ...
      const pulse =
        1 +
        Math.sin(t * 1.7 + dance) * 0.045 +
        Math.sin(t * 4.2 + dance) * 0.018;
    ...
    if (isPressing && pressOnLemon) {
      ...
      if (isInsideLemon(world.x, world.y)) {
        ...
        if (sacredDragTimer >= 0.38 && 1 - probOrganic > 0.44) {
          sacredDragTimer = 0;
          if (Math.random() < 0.7) {
            emitSacred(world.x, world.y, 1, 1 - probOrganic);
          }
        }
      } else {
        dragEmitAccumulator = 0;
